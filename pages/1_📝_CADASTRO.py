...
with st.form("form_cadastro"):
    ...
    with col2:
        ...
        tipo_documento = st.selectbox("Tipo de Documento", import streamlit as st
import uuid
import time
import json
from pathlib import Path
...
def sugerir_metadados(texto_pdf):
    prompt = f"""
    Extraia os metadados do seguinte texto. Responda APENAS em JSON:
    "titulo", "autores", "ano", "pais", "resumo", "palavras_chave", "instituicao", "idioma".
    Texto: {texto_pdf[:4000]}
    """
    try:
...
with st.form("form_cadastro"):
    ...
    # Linha 2: Autores e Instituição
    col_aut, col_inst = st.columns(2)
    with col_aut:
        autores = st.text_input("Autor(es)", value=st.session_state.form_data["autores"])
    with col_inst:
        instituicao = st.text_input("Instituição", value=st.session_state.form_data["instituicao"])

    # Linha 3: Tipo, País e Idioma
    ...
    with col_tipo:
        tipo_documento = st.selectbox("Tipo de Documento ["",    "Artigo",
                ...
                "Outros", "Outro"])
    with col_pais:
        pais = st.selectbox("País", ["", "Brasil", "Argentina", "Chile", "Espanha", "EUA", "Portugal", "Outro"])
    with col_idioma:
        idioma = st.selectbox("Idioma", ["Português", "Inglês", "Espanhol", "Outro"])

    # Temas e Subtemas (Largura Total)
    tema = st.selectbox("Tema", ["", "Financiamento", "Tarifa (Taxas de drenagem)", "Custos (Operacionais e de Implantação)","Taxas","Regulação e Governança","Soluções Baseadas na Natureza (SbN) e Infraestrutura Verde", "Planejamento Urbano e Uso do Solo","Sustentabilidade e Mudanças Climáticas","Tecnologias de Monitoramento", "Cidades Inteligentes (Smart Cities)",           "Investimentos em DMAPU","Cidades Inteligentes","Manejo de Águas Pluviais Urbanas (MAPU)",
...
if submitted:
    if not titulo:
        ...
        try:
            ...
            # Chamada da função ajustada para as variáveis que realmente existem no formulário
            inserir_documento(
                titulo=titulo, 
                autores=autores, 
                ano=ano, 
            ...
            st.session_state.form_data = {"titulo": "", "autores": "", "ano": 2025, "resumo": "", "palavras_chave": "", "instituicao": "", "idioma": "Português", "pais": ""}
            time.sleep(2)
            st.rerun()
        except Exception as e:
